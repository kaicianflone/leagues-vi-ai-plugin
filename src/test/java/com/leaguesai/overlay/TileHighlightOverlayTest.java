...
public class TileHighlightOverlayTest {
    ...
    @Test
    public void render_whenTargetInScene_drawsPolygon() {
        overlay.setTargetTile(new WorldPoint(3200, 3200, 0));
        BufferedImage img = new BufferedImage(800, 600, BufferedImage.TYPE_INT_ARGB);
        Graphics2D g = spy(img.createGraphics());

        LocalPoint mockLp = new LocalPoint(100, 100);
        Polygon mockPoly = new Polygon(new int[]{0, 10, 10, 0}, new int[]{0, 0, 10, 10}, 4);
        ...
        try (MockedStatic<LocalPoint> lpMock = mockStatic(LocalPoint.class);
            ...
            perspMock.when(() -> Perspective.getCanvasTilePoly(eq(client), eq(mockLp))).thenReturn(mockPoly);

            assertNull(overlay.render(g));

            // OverlayUtil.renderPolygon calls graphics.fill(shape) and graphics.draw(shape)
